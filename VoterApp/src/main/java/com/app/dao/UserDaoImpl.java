...
public class UserDaoImpl implements UserDao {
	...
	public UserDaoImpl() throws SQLException {
		...
		pst2 = cn.prepareStatement("select * from user where role='voter' and dob between ? and ?");
		// pst3 : voter reg
		/*
		 * id | first_name | last_name | email | password | dob | status | role
		 */
		pst3 = cn.prepareStatement("insert into user values(default,?,?,?,?,?,?,?)");
		// pst4 --change pwd
		pst4 = cn.prepareStatement("update user set password=? where email=? and password=? and role='voter'");
		//update voting status
				pst5=cn.prepareStatement("update user set status=1 where userId=?");
	...
	@Override
	public int validateAge(String dob) throws SQLException {
		DateTimeFormatter formater=DateTimeFormatter.ofPattern("yyyy-MM-dd");
		LocalDate dateofbirth=LocalDate.parse(dob,formater);
